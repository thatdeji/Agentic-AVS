...
library AnalysisDeploymentLib {
    ...
    struct DeploymentData {
        address analysisServiceManager;
    ...
    function deployContracts(
        ...
        result.analysisServiceManager = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        result.stakeRegistry = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        // Deploy the implementation contracts, using the proxy contracts as inputs
        address stakeRegistryImpl =
            address(new ECDSAStakeRegistry(IDelegationManager(core.delegationManager)));
        address analysisServiceManagerImpl = address(
            new AnalysisServiceManager(
                core.avsDirectory, result.stakeRegistry, core.rewardsCoordinator, core.delegationManager
            )
        );
        // Upgrade contracts
        bytes memory upgradeCall = abi.encodeCall(
            ECDSAStakeRegistry.initialize, (result.analysisServiceManager, 0, quorum)
        );
        UpgradeableProxyLib.upgradeAndCall(result.stakeRegistry, stakeRegistryImpl, upgradeCall);
        upgradeCall = abi.encodeCall(AnalysisServiceManager.initialize, (owner, rewardsInitiator));
        UpgradeableProxyLib.upgradeAndCall(result.analysisServiceManager, analysisServiceManagerImpl, upgradeCall);
    ...
    function readDeploymentJson(
        ...
        require(vm.exists(fileName), "AnalysisDeployment: Deployment file does not exist");

        string memory json = vm.readFile(fileName);

        DeploymentData memory data;
        /// TODO: 2 Step for reading deployment json.  Read to the core and the AVS data
        data.analysisServiceManager = json.readAddress(".addresses.analysisServiceManager");
    ...
    function writeDeploymentJson(
        ...
        writeDeploymentJson("deployments/analysis/", block.chainid, data);
    }
    ...
    function writeDeploymentJson(
        ...
        address proxyAdmin =
            address(UpgradeableProxyLib.getProxyAdmin(data.analysisServiceManager));
    ...
    function readDeploymentConfigValues(
        ...
        require(vm.exists(pathToFile), "AnalysisDeployment: Deployment Config file does not exist");
    ...
    function _generateContractsJson(
        ...
        return string.concat(
            ...
            data.analysisServiceManager.toHexString(),
            '","analysisServiceManagerImpl":"',
            data.analysisServiceManager.getImplementation().toHexString(),
            '","stakeRegistry":"',
