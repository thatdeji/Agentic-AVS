...
contract SetupPayments is Script, Test {
    ...
    function setUp() public {
        ...
        AnalysisDeployment = AnalysisDeploymentLib.readDeploymentJson("deployments/analysis/", block.chainid);
    ...
    function run() external {
        vm.startBroadcast(AnalysisConfig.rewardsInitiatorKey);
    ...
    function executeProcessClaim() public {
        ...
        vm.startBroadcast(deployer);
        earnerLeaves = _getEarnerLeaves(_getEarners(deployer), amountPerPayment, AnalysisDeployment.strategy);
    ...
    function createAVSRewardsSubmissions(uint256 numPayments, uint256 amountPerPayment, uint32 startTimestamp) public {
        ERC20Mock(AnalysisDeployment.token).mint(AnalysisConfig.rewardsInitiator, amountPerPayment * numPayments);
        ERC20Mock(AnalysisDeployment.token).increaseAllowance(AnalysisDeployment.analysisServiceManager, amountPerPayment * numPayments);
        uint32 duration = rewardsCoordinator.MAX_REWARDS_DURATION();
        SetupPaymentsLib.createAVSRewardsSubmissions(
            AnalysisDeployment.analysisServiceManager,
            AnalysisDeployment.strategy,
            numPayments,
            amountPerPayment,
            duration,
            startTimestamp
