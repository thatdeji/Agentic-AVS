...
const uniswapClient = new GraphQLClient(`https://gateway.thegraph.com/api/${process.env.GRAPH_API_KEY}/subgraphs/id/${UNISWAP_V3}`);
...
const hopProtocolClient = new GraphQLClient(`https://gateway.thegraph.com/api/${process.env.GRAPH_API_KEY}/subgraphs/id/${HOP_PROTOCOL}`);
...
const ensClient = new GraphQLClient(`https://gateway.thegraph.com/api/${process.env.GRAPH_API_KEY}/subgraphs/id/${ENS_PROTOCOL}`);
...
(async () => {
    const res = await getEnsDomainsByAddress("0xcd4bde67fe7c6eb601d03a35ea8a55eb2b136965");
    console.log(res)
})()

export {getUniswapV3SwapsByAddress, getEnsDomainsByAddress, getHopProtocolBridgeTransfersByAddress};